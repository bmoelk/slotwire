export function fieldDefinitionToSonicSchema(field) {
    const schema = {
...
export function generateSonicCollectionConfig(slotKey, slotDef) {
...
export function generateSonicTicketCollectionConfig() {
    return {
...
export function generateSonicNavigationCollectionConfig(config) {
...
export function scaffoldAllSonicCollections(config) {
    const collections = [];
 ...
 */
export async function scaffoldBlueprint(blueprint, options = {}) {
    const { cmsApiUrl = 'https://cms.example.com', apiKey, d1, } = options;
    const itemsToCreate = (blueprint.items || []).filter((i) => i.action === 'create');
    const createdIds = [];
    const errors = [];
    for (const item of itemsToCreate) {
        try {
            if (d1) {
                ...
                await d1
                    .prepare(`INSERT INTO ${item.collection} (id, data, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)
             ON CONFLICT(id) DO UPDATE SET data = excluded.data, updated_at = excluded.updated_at`)
                    ...
                    await d1
                        .prepare(`INSERT OR REPLACE INTO ${item.collection} (id, slug, page_slug, section_key, title, status)
                 VALUES (?, ?, ?, ?, ?, ?)`)
                        .bind(recordId, item.data.slug || item.pageSlug, item.pageSlug, item.sectionKey || 'default', item.data.title || 'Draft', 'draft')
                        .run();
                });
