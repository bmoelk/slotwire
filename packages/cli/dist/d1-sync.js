...
export function generateD1ViewsSql(config, options = {}) {
    ...
    if (config.slots) {
        for (const [key, def] of Object.entries(config.slots)) {
            const colName = def.collectionName || def.collection || key;
        ...
        for (const [, arch] of Object.entries(config.archetypes)) {
            ...
            if (arch.slots) {
                for (const [, slotDef] of Object.entries(arch.slots)) {
                    const colName = slotDef.collection || 'page_sections';
                    if (!collectionsMap.has(colName)) {
                        collectionsMap.set(colName, new Set());
                    }
                    const fieldSet = collectionsMap.get(colName);
                    if (slotDef.defaultData) {
                        for (const k of Object.keys(slotDef.defaultData)) {
 ...
 */
export async function syncD1Views(options = {}) {
...
//# sourceMappingURL=d1-sync.js.map