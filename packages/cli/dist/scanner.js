 ...
 */
export function extractSlotsFromHtml(html) {
    const slots = [];
    ...
    while ((match = elementRegex.exec(html)) !== null) {
        ...
        const isGhost = /data-slotwire-ghost=["']true["']/i.test(fullAttributes) ||
 ...
 */
export async function scanHtmlDirectory(distDir, options = {}) {
    const resolvedDir = path.resolve(distDir);
    const errors = [];
    const routes = [];
    async function findHtmlFiles(currentDir) {
        const files = [];
        try {
        ...
        try {
            ...
            if (slots.length > 0) {
                ...
                if (options.strict && ghostSlots > 0) {
                    errors.push(`Route '${route}' has ${ghostSlots} unpopulated ghost slot(s): ${slots.filter((s) => s.isGhost).map((s) => s.slot).join(', ')}`);
                }
                if (options.strict && fallbackSlots > 0) {
                    errors.push(`Route '${route}' has ${fallbackSlots} slot(s) relying on static fallback data instead of live CMS records: ${slots.filter((s) => s.source === 'fallback').map((s) => s.slot).join(', ')}`);
                }
                routes.push({
                    route,
