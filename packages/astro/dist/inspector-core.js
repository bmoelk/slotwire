...
export function renderInspectorHtml(options = {}) {
    ...
    const rawProvider = options.provider ||
        ...
        globalThis?.__SLOTWIRE_CONFIG__?.cms?.provider ||
        'cms';
...
export function initInspector(containerEl, options = {}) {
    const adminUrl = options.adminUrl || '/admin';
    const siteId = options.siteId || '';
    const siteQuery = siteId ? `?site_id=${encodeURIComponent(siteId)}&site=${encodeURIComponent(siteId)}` : '';
    let areOverlaysVisible = sessionStorage.getItem('slotwire_hide_overlays') !== 'true';
    let isHighlightActive = (typeof document !== 'undefined' && document.documentElement.classList.contains('slotwire-highlight-slots')) ||
        sessionStorage.getItem('slotwire_highlight_slots') === 'true';
    let activeFilter = 'all';
    let searchQuery = '';
    // 1. Overlay Visibility Controller
    function syncOverlayState(visible) {
    ...
    function syncHighlightState(active) {
        ...
        const staticSlots = document.querySelectorAll('[data-slotwire-slot]:not(.slotwire-slot-container)');
        staticSlots.forEach((el) => {
            let badge = el.querySelector(':scope > .slotwire-static-badge');
            if (active) {
                if (!badge) {
    ...
    function scanAndRender() {
        const slotElements = Array.from(document.querySelectorAll('.slotwire-slot-container, [data-slotwire-slot]'));
        // Deduplicate nested elements
        const seenSlots = new Set();
        const uniqueElements = [];
        slotElements.forEach((el) => {
            const container = el.closest('.slotwire-slot-container') || el;
        ...
        if (tabSlotsCount)
            tabSlotsCount.textContent = String(totalSlots);
        ...
        const fillEl = containerEl.querySelector('#sw-progress-fill');
        const dividerEl = containerEl.querySelector('#sw-progress-divider');
        const completenessText = `Completeness: ${pct}% (${populatedSlots}/${totalSlots} populated)`;
        if (pctEl)
            pctEl.textContent = completenessText;
        if (fillEl)
            fillEl.style.width = `${pct}%`;
        if (dividerEl)
            dividerEl.setAttribute('title', completenessText);
        ...
        function updateTelemetry() {
            ...
            if (telDom)
                telDom.textContent = domMs > 0 ? (domMs >= 1000 ? `${(domMs / 1000).toFixed(2)}s` : `${domMs}ms`) : 'ready';
            if (telReqs)
                telReqs.textContent = `${reqCount}`;
            if (telSlots)
                telSlots.textContent = `${totalSlots}`;
            // Detailed diagnostics breakdown
            const diagRoute = containerEl.querySelector('#sw-diag-route');
            if (diagRoute && typeof window !== 'undefined')
                diagRoute.textContent = window.location.pathname || '/';
      ...
      `)
        ...
        listEl.querySelectorAll('.sw-locate-btn').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                const targetIdx = parseInt(e.target.getAttribute('data-locate-idx') || '0', 10);
    ...
    const tabBtns = containerEl.querySelectorAll('.sw-tab-btn');
    const tabPanes = containerEl.querySelectorAll('.sw-tab-pane');
    ...
    window.addEventListener('slotwire:highlight-change', ((e) => {
    ...
    const copyBtn = containerEl.querySelector('#sw-btn-copy-report');
    if (copyBtn) {
    ...
    const searchEl = containerEl.querySelector('#sw-filter-search');
    if (searchEl) {
...
export function openSlotRequestModal(info) {
          ...
          <div style="padding:20px; display:flex; flex-direction:column; gap:14px; font-size:12px;">
            ...
            <div style="background:#18181b; border:1px solid #27272a; border-radius:8px; padding:12px;">
              ...
              <div style="display:flex; flex-wrap:wrap; gap:6px;">
                ${info.hierarchy
            .map((item, idx) => `
                  <button class="sw-hierarchy-chip" data-idx="${idx}" style="background:${idx === activeIndex ? '#065f46' : '#27272a'}; border:1px solid ${idx === activeIndex ? '#10b981' : '#3f3f46'}; color:${idx === activeIndex ? '#ffffff' : '#d4d4d8'}; border-radius:6px; padding:4px 8px; font-size:11px; font-family:monospace; cursor:pointer; display:inline-flex; align-items:center; gap:4px;">
                    ${idx === 0 ? '🎯 ' : '↳ '}${item.label}
                  </button>
                `)
            .join('')}
    ...
    `;
        const archetypeSelect = modalRoot.querySelector('#sw-input-archetype');
        const collectionInput = modalRoot.querySelector('#sw-input-collection');
        const archetypeToCollectionMap = {
        ...
        function flashElement(el) {
            const backdrop = modalRoot.querySelector('#sw-modal-backdrop');
            if (backdrop) {
        ...
        modalRoot.querySelectorAll('.sw-hierarchy-chip').forEach((chip) => {
            chip.addEventListener('click', (e) => {
                activeIndex = parseInt(e.currentTarget.getAttribute('data-idx') || '0', 10);
                renderModalContent();
            });
        });
        ...
        modalRoot.querySelector('#sw-modal-submit-btn')?.addEventListener('click', async () => {
            const slotInput = modalRoot.querySelector('#sw-input-slot')?.value.trim();
            if (!slotInput) {
                alert('Please specify a Slot Key.');
                return;
            }
            const slotKey = slotInput.toLowerCase().replace(/[^a-z0-9_]/g, '_');
            const collection = modalRoot.querySelector('#sw-input-collection')?.value.trim() || 'page_sections';
            const archetype = modalRoot.querySelector('#sw-input-archetype')?.value || 'section';
            const notes = modalRoot.querySelector('#sw-input-notes')?.value.trim() || '';
            const shouldDispatch = modalRoot.querySelector('#sw-input-dispatch')?.checked;
            const ticketPayload = {
                version: '1.0.0',
...
export function startVisualElementPicker(onDone) {
  ...
  `;
    ...
    function onMouseMove(e) {
        ...
        if (!target || target.closest('#sw-picker-banner') || target.closest('astro-dev-toolbar') || target.closest('#slotwire-assist-container'))
            return;
...
//# sourceMappingURL=inspector-core.js.map