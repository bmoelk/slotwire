 ...
 */
export async function handleScaffoldRequest(request, options = {}) {
    ...
    const targetSlug = String(body.targetSlug || body.pageSlug || body.slug || '').trim().replace(/^\/+|\/+$/g, '');
    const targetTitle = String(body.targetTitle || body.title || '').trim() || targetSlug.replace(/[-_]/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
 ...
 */
export function createScaffoldEndpoint(options = {}) {
