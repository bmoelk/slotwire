...
function insertText(textarea: HTMLTextAreaElement, before: string, after: string = '', multiline: boolean = false): void {
  textarea.focus();
...
export class MarkdownToolbarElement extends BaseElement {
  ...
  private handleClick(event: MouseEvent) {
    const target = (event.target as HTMLElement).closest('button, [data-md-action]');
    if (!target) return;
