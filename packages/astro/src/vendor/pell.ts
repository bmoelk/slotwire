...
export function exec(command: string, value: string | null = null): boolean {
  return document.execCommand(command, false, value as any);
}
...
export const defaultActions: Record<string, PellAction> = {
  bold: {
    name: 'bold',
    icon: '<b>B</b>',
    title: 'Bold (Cmd+B)',
...
export function initPell(options: PellOptions): PellEditorInstance {
  const {
    element,
    onChange,
  ...
  if (styleWithCSS) exec('styleWithCSS');
