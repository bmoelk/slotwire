...
export class WordPressAdapter extends BaseCmsAdapter {
  ...
  private mapCollectionToWpRoute(collection: string): string {
    ...
    if (clean === 'pages' || clean === 'page') return 'pages';
    if (clean === 'posts' || clean === 'post' || clean === 'blog_posts' || clean === 'blog_post' || clean === 'blog') return 'posts';
    if (clean === 'categories' || clean === 'category') return 'categories';
    if (clean === 'tags' || clean === 'tag') return 'tags';
    if (clean === 'media' || clean === 'attachment' || clean === 'attachments') return 'media';
    if (clean === 'site_navigation' || clean === 'menu' || clean === 'menus' || clean === 'navigation') return 'menu-items';
    return clean;
  }
  ...
  async fetchCollection(
    ...
    options: {
      ...
      sort?: any;
      limit?: number;
      credentials?: { apiUrl?: string; apiKey?: string; previewToken?: string };
    } = {}
  ): Promise<any[]> {
    ...
    if (options.filter?.slug) {
      const slugVal =
        typeof options.filter.slug === 'object'
