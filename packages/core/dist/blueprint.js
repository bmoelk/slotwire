 ...
 */
export function generateBlueprint(config, archetypeKey, options) {
    const { targetSlug, targetTitle = targetSlug.replace(/[-_]/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()), template, addToMenu, menuKey = 'header_main', menuLabel, menuOrder = 20, parentSlug, customData = {}, } = options;
    const archetype = config.archetypes?.[archetypeKey] || {
        name: archetypeKey,
        collection: 'pages',
...
function processSlotDefinition(slotDef, slotKey, pageSlug, pageTitle, items, depth, parentKey) {
    const sectionKey = slotDef.key || slotKey;
    const isReference = slotDef.strategy === 'reference' ||
    ...
    if (slotDef.children) {
        for (const [childKey, childDef] of Object.entries(slotDef.children)) {
            ...
            const childCollection = childDef.collection || childDef.collectionName || childKey;
