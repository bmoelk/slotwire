...
export class BaseGitAdapter extends BaseCmsAdapter {
    resolveEntrySlug(documentId, pageSlug, sectionKey) {
        if (documentId)
            return documentId;
...
export class KeystaticAdapter extends BaseGitAdapter {
    ...
    buildAdminLink(options) {
        const { adminUrl = '/keystatic', collection = '', documentId, pageSlug, sectionKey, action = documentId ? 'edit' : 'list', archetype, } = options;
...
export class DecapAdapter extends BaseGitAdapter {
    provider = 'decap';
    buildAdminLink(options) {
        const { adminUrl = '/admin', collection = '', documentId, pageSlug, sectionKey, action = documentId ? 'edit' : 'list', archetype, } = options;
        const base = this.cleanBaseUrl(adminUrl);
        const targetSlug = this.resolveEntrySlug(documentId, pageSlug, sectionKey);
        const isCollectionArchetype = archetype === 'cards' ||
            archetype === 'gallery' ||
